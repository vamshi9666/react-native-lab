...
const CarouselContainer = ({ width, height, children }) => {
  return (
    <View
      style={{
        flex: 1,
    ...
    ...cumulative.map((element, index) =>
      block([
        cond(
          lessOrEq(
            cond(
              and(eq(canScroll, 1), greaterThan(abs(dragX), 5)),
              add(offsetX, getScrollBehaviour(scrollSpeed)),
              add(offsetX, dragX)
            ),
            cond(
              and(eq(index, 0), eq(canScroll, 1)),
              abs(firstIndexWidth),
              element
            )
          ),
...
function setTranslationX(
  ...
  return (translationX = cond(
    ...
    [
      setIndexFromWidthArray(cumulative, canScroll, scrollSpeed),
      add(offsetX, dragX)
    ],
    runTiming(
      new Clock(),
...
class Carousel extends Component {
  ...
  addLayoutValues = () => {
    ...
    const withPreviewArray =
      withPreview && this.withPreviewArray(cumulativeArray);
  ...
  onLayout = (event, index) => {
    this.setState({
      widthArrays: [
        ...this.state.widthArrays,
        event.nativeEvent.layout.width * -1
      ]
    });
      ...
      >
        ...
        >
          ...
          <Animated.View
            style={{
              transform: [
                { translateX: this.state.isLoading ? 0 : this.transX }
              ]
            }}
          >
            <CarouselContainer
